...
def log(info):
    line = f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] {info}"
    print(line)
    msg_list.append(line)

def send_tg_msg(content):
    if TG_BOT_TOKEN and TG_USER_ID:
        url = f"https://api.telegram.org/bot{TG_BOT_TOKEN}/sendMessage"
        data = {"chat_id": TG_USER_ID, "text": f"🤖 EUserv 自动续期报告\n\n" + "\n".join(content)}
        try: requests.post(url, data=data, timeout=10)
        except: print("TG 通知发送失败")

def get_totp(key, time_step=None):
    try:
        if time_step is None: time_step = int(time.time() / 30)
            ...
            for oid in orders:
                ...
                if pin:
                    ...
                    if token:
                        ...
                        log(f"🎉 订单 {oid} 续期成功！")
                else: log(f"❌ 订单 {oid} 未获 PIN")
        else:
            log("❌ 登录失败，请检查账号或等待 IP 释放")
    except Exception as e:
        log(f"💥 运行异常: {str(e)[:50]}")
    
    send_tg_msg(msg_list)
